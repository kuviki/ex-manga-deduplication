...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QLabel, QCheckBox, QMenu, QMessageBox, QHeaderView
)
...
from ..core.scanner import DuplicateGroup, ComicInfo
...
class DuplicateListWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.delete_selected_btn.setStyleSheet("QPushButton { background-color: #ff6b6b; color: white; }")
    ...
    def refresh_list(self):
        ...
        for i, group in enumerate(self.duplicate_groups, 1):
            ...
            for comic in group.comics:
                ...
                # 存储漫画数据
                comic_item.setData(0, Qt.UserRole, {
                    'type': 'comic',
                    'comic': comic,
                    'group': group
                })
        ...
        # 更新统计信息
        self.stats_label.setText(f"{len(self.duplicate_groups)} 组重复，共 {total_comics} 个文件")
    ...
    def show_context_menu(self, position):
        ...
        open_location_action.triggered.connect(lambda: self.open_file_location(comic.path))
    ...
    def refresh_after_deletion(self, deleted_paths: List[str]):
        ...
        for group in self.duplicate_groups[:]:
            group.comics = [comic for comic in group.comics if comic.path not in deleted_paths]
