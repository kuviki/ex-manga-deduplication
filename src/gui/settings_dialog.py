...
from PyQt5.QtWidgets import (
    ...
    QFormLayout, QLabel, QSpinBox, QComboBox, QLineEdit,
    QPushButton, QFileDialog, QCheckBox, QGroupBox,
    QDialogButtonBox, QMessageBox, QSlider, QTextEdit
)
...
class SettingsDialog(QDialog):
    ...
    def create_hash_tab(self):
        ...
        for algo in HashAlgorithm:
            self.hash_algorithm_combo.addItem(self._get_algorithm_display_name(algo), algo.value)
        ...
        for algo in HashAlgorithm:
            ...
            threshold_layout.addRow(f"{self._get_algorithm_display_name(algo)}:", spinbox)
    ...
    def create_app_tab(self):
        ...
        for handling in ErrorHandling:
            self.error_handling_combo.addItem(self._get_error_handling_display_name(handling), handling.value)
    ...
    def apply_settings(self):
        ...
        try:
            ...
            # 检测设置
            self.config.set("min_similar_images", self.min_similar_images_spinbox.value())
            self.config.set("min_image_resolution.width", self.min_width_spinbox.value())
            self.config.set("min_image_resolution.height", self.min_height_spinbox.value())
            ...
            self.config.set("window_geometry.height", self.window_height_spinbox.value())
