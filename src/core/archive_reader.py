...
class ArchiveReader:
    ...
    def read_all_images(self, archive_path: str) -> Generator[Tuple[str, bytes], None, None]:
        """读取压缩包中的所有图片
...
class ArchiveCache:
    ...
    def remove_old_entries(self, archive_path: str) -> None:
        """移除指定压缩包的旧缓存条目"""
        keys_to_remove = [key for key in self._cache.keys() if key.startswith(f"{archive_path}:")]
