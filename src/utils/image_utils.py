...
def resize_image(image_data: bytes, max_size: Tuple[int, int], 
                keep_aspect_ratio: bool = True, quality: int = 85) -> Optional[bytes]:
    """调整图像大小"""
    try:
        with Image.open(io.BytesIO(image_data)) as img:
            # 转换为RGB模式（如果需要）
...
def normalize_image_for_hash(image_data: bytes, size: Tuple[int, int] = (32, 32)) -> Optional[Image.Image]:
    """标准化图像用于哈希计算"""
    try:
        with Image.open(io.BytesIO(image_data)) as img:
            # 转换为灰度
...
def convert_image_format(image_data: bytes, target_format: str, quality: int = 85) -> Optional[bytes]:
    """转换图像格式"""
    try:
        with Image.open(io.BytesIO(image_data)) as img:
            # 如果目标格式不支持透明度，转换为RGB
