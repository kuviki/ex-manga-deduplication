...
def create_test_data(
    num_comics: int = 100, images_per_comic: int = 20
) -> List[ComicInfo]:
    """创建测试数据"""
        ...
        for j in range(images_per_comic):
            ...
            if i < 10 and j < 5:  # 前10个漫画的前5张图片设为相似
                ...
                flip_positions = np.random.choice(
                    64, np.random.randint(1, 4), replace=False
                )
                base_hash[flip_positions] = 1 - base_hash[flip_positions]
                hash_str = "".join(
                    [
                        hex(int("".join(map(str, base_hash[k : k + 4])), 2))[2:]
                ...
                hash_str = "".join(
                    [
                        hex(int("".join(map(str, random_hash[k : k + 4])), 2))[2:]
                        for k in range(0, 64, 4)
                    ]
                )
...
def benchmark_detection(comic_infos: List[ComicInfo], config: ConfigManager) -> float:
    ...
    logger.info(
        f"  - 总图片数: {sum(len(comic.image_hashes) for comic in comic_infos)}"
    )
    logger.info(f"  - 重复组数: {len(duplicate_groups)}")
    logger.info(
        f"  - 平均每个漫画处理时间: {elapsed_time / len(comic_infos) * 1000:.2f} 毫秒"
    )
